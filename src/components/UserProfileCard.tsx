...
import { X, MessageSquare, UserPlus, UserX, Shield, Flag, Star, Zap, Award } from 'lucide-react';
        ...
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
          ...
          <div className="px-5 pb-5">
            <div className="flex items-end justify-between -mt-10 mb-3">
              <div className="w-16 h-16 rounded-2xl border-4 border-[#111418] bg-white/10 flex items-center justify-center text-xl font-black text-white/60 overflow-hidden">
            ...
            <div className="mb-3">
              <div className="flex items-center gap-2">
                <h3 className="text-white font-bold text-lg">{target.username}</h3>
                {target.is_admin && <span className="text-[10px] px-1.5 py-0.5 bg-yellow-500/20 text-yellow-400 rounded font-bold">ADMIN</span>}
                {isFriend && <span className="text-[10px] px-1.5 py-0.5 bg-emerald-500/20 text-emerald-400 rounded">Arkadaş</span>}
              </div>
              <p className="text-white/40 text-xs mt-0.5">{target.status === 'online' ? '🟢 Çevrimiçi' : '⚫ Çevrimdışı'}</p>
            ...
            <div className="grid grid-cols-2 gap-2 mb-3">
              <div className="bg-white/5 rounded-xl p-2 text-center">
                ...
                <p className="text-white/30 text-xs">Mesaj</p>
              </div>
              <div className="bg-white/5 rounded-xl p-2 text-center">
                <p className="text-white font-bold text-lg">{level}</p>
                <p className="text-white/30 text-xs">Seviye</p>
              </div>
            </div>
            {/* Badges */}
            {badges.length > 0 && (
