...
import { X, Palette, Type, Bell, BellOff, Monitor, Moon, Zap, Check, Layout, Volume2, BadgeCheck, Clock, CheckCircle2, AlertTriangle } from 'lucide-react';
...
export const UserSettings = ({
  ...
  const [tab, setTab] = useState<'appearance' | 'notifications' | 'accessibility' | 'verify'>('appearance');
        ...
        <div className="w-44 border-r border-white/5 p-3 flex flex-col gap-1">
          ...
          {TABS.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)}
              className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-all text-left ${tab === t.id ? 'bg-emerald-500/20 text-emerald-400 font-bold' : 'text-white/50 hover:text-white hover:bg-white/5'}`}>
