...
import {
  ref, onValue, push, set, get, update, remove, off, query, orderByChild, limitToLast
...
export async function createChannel(name: string, type = 'text', category = 'Sohbet') {
  const id = name.toLowerCase().replace(/\s+/g, '-');
  await set(ref(db, `channels/${id}`), { name, type, category, server_id: 'main', is_locked: false, slow_mode: 0 });
  return { id, name, server_id: 'main', type, category };
}
...
export async function sendMessage(channelId: string, senderId: string, senderName: string, content: string, options?: {
  ...
  const userRef = ref(db, `users/${senderId}/message_count`);
  const snap = await get(userRef);
  await set(userRef, (snap.val() || 0) + 1);
  return msgRef.key;
}
...
export function listenUsers(callback: (users: any[]) => void) {
  ...
  onValue(r, snap => {
    ...
    // UID gibi görünen key'i tespit et (28 karakter alfanumerik)
    const looksLikeUID = (key: string) => /^[A-Za-z0-9]{20,}$/.test(key);

    Object.entries(data).forEach(([id, val]: any) => {
      if (!val || typeof val !== 'object') return;

      // Eğer val.id varsa bu yeni UID-based kayıt, val.username kullan
      // Eğer val.id yoksa bu eski username-keyed kayıt, key = username
      const username = val.username || (looksLikeUID(id) ? null : id);
      if (!username) return; // UID-keyed kayıt ama username yoksa atla

      const lc = username.toLowerCase();
      const isNewRecord = !!val.id; // Yeni kayıtlarda id field var

      if (!byUsername.has(lc)) {
        byUsername.set(lc, { id, username, val });
      } else {
        // Aynı kullanıcı için yeni kayıt (uid-keyed, val.id var) eskiye tercih edilir
        if (isNewRecord) {
          byUsername.set(lc, { id, username, val });
        }
      }
...
export async function acceptFriendRequest(userId: string, fromId: string, requestId: string) {
  await remove(ref(db, `friend_requests/${userId}/${requestId}`));
